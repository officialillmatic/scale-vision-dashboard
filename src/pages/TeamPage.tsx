import React, { useState, useEffect       </div>
    </DashboardLayout>
  );
};

export default TeamPage; from 'react';
...
export default function TeamPage() {
  ...
  const fetchTeamMembers = async () => {
    try {
      console.log('🔍 === ANÁLISIS PROFUNDO DE USUARIOS ===');
      
      // 1. Obtener TODOS los usuarios de auth.users (tabla principal de Supabase)
      const { data: authUsers, error: authError } = await supabase.auth.admin.listUsers();
      console.log('👥 Usuarios en auth.users:', authUsers?.users?.length || 0);
      
      // 2. Obtener usuarios de la tabla public.users
      const { data: publicUsers, error: publicError } = await supabase
        .from('users')
        .select('*');
      console.log('👥 Usuarios en public.users:', publicUsers?.length || 0);

      // 3. Comparar ambas fuentes
      if (authUsers?.users) {
        console.log('📧 Emails en auth.users:', authUsers.users.map(u => u.email));
      }
      if (publicUsers) {
        console.log('📧 Emails en public.users:', publicUsers.map(u => u.email));
      }
      ...
      console.log('🔍 Solo en auth.users:', onlyInAuth);
      console.log('🔍 Solo en public.users:', onlyInPublic);

      // 5. Usar la fuente más completa
      let allUsersData = [];
      
      if (authUsers?.users && authUsers.users.length > 0) {
        // Convertir usuarios de auth a formato compatible
        allUsersData = authUsers.users.map(authUser => {
          ...
          return {
            ...
            full_name: authUser.user_metadata?.full_name || publicUser?.full_name,
            created_at: authUser.created_at || publicUser?.created_at,
            source: publicUser ? 'both' : 'auth_only'
          };
        });
        console.log('✅ Usando datos combinados de auth.users + public.users');
      } else if (publicUsers && publicUsers.length > 0) {
        allUsersData = publicUsers.map(user => ({ ...user, source: 'public_only' }));
        console.log('✅ Usando datos de public.users únicamente');
      } else {
        console.log('❌ No se encontraron usuarios en ninguna tabla');
        setTeamMembers([]);
        return;
      }

      console.log('📊 Total usuarios detectados:', allUsersData.length);
      console.log('📋 Detalle de fuentes:', allUsersData.map(u => ({ email: u.email, source: u.source })));
      ...
      // 6. Obtener datos complementarios
      const userIds = allUsersData.map(u => u.id).filter(Boolean);
      console.log('🔑 IDs para consultas adicionales:', userIds.length);
      ...
      console.log('  - Perfiles:', profilesData.length);
      console.log('  - Créditos:', creditsData.length);

      // 7. Obtener nombres de empresas
      ...
      const combinedMembers: TeamMember[] = allUsersData.map(user => {
        const profile = profilesData.find(p => p.id === user.id || p.email === user.email);
        ...
        console.log(`👤 Usuario procesado: ${memberData.email} (${user.source}) - Team: ${teamStatus}`);
        return memberData;
      });

      setTeamMembers(combinedMembers);
      console.log('✅ === ANÁLISIS COMPLETADO ===');
      console.log(`📊 Total miembros cargados: ${combinedMembers.length}`);
      console.log('📋 Lista final:', combinedMembers.map(m => ({ email: m.email, team_status: m.team_status })));
